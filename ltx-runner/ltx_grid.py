...
def auto_place(count: int, num_frames: int) -> list[int]:
    ...
    if count == 1:
        return [0]
    # Terminal index sits AT num_frames, not before it: run_richmond.sh used
    # `--image kf4.png 121` with `--num-frames 121` and that is the configuration
    # that produced working video. Positions are continuous time, so a terminal
    # token just past the last frame still pulls the tail onto the final beat --
    # and pulling it closer is exactly the recipe's fix for the model improvising
    # in the gap after the last waypoint.
    last = num_frames if is_on_grid(num_frames) else snap(num_frames - 1)
    out = [0]
